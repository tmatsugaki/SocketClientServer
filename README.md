...
カーネルやローダブルモジュールの再構築が必要なのですが、Linux の V4L2 の GStream に関わる部分はデリケートなので熟慮が必要です。
Windows11 の WSL2 で費やしたことが徒労に終わる可能性がありますから、クロス開発環境としては、デバイスを楽に扱える PC上のネイティブ ubuntu 若しくは macOS を使用することを推奨します。
